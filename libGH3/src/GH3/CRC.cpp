...
namespace GH3
{
    CRC::ChecksumName* CRC::ChecksumNameHashTable = reinterpret_cast<CRC::ChecksumName*>(0x00c0775c);
    char* CRC::ChecksumNames = reinterpret_cast<char*>(0x00c07760);

    uint32_t CRC::CRC32TableLength = 256;
    uint32_t* CRC::CRC32Table = reinterpret_cast<uint32_t*>(0x0095CD28);

    CRCKey CRC::CreateKeyFromStringOut(CRCKey* outKey, const char* srcString)
    {
    ...
    CRCKey CRC::CreateKeyFromString(const char* srcString)
    {
        return CreateKeyFromStringOut(nullptr, srcString);
    }


    void CRC::CreateKeyNameAssociation(CRCKey key, char* name)
    {
        reinterpret_cast<void(__cdecl*)(CRCKey, char*)>(key, name);
    }

    char* CRC::FindChecksumName(CRCKey key)
    {
        return reinterpret_cast<char* (*)(CRCKey)>(0x004a69a0)(key);
    }

    // V: End of Experimental
